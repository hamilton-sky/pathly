---
name: pathly
description: Codex-safe Pathly entry point. Route /pathly or /path help, doctor, debug, explore, flow, review, continue, plan, build, archive, lessons, imports, verify-state, or plain-English requests to the right Pathly workflow without using Codex built-in /help.
...
Codex users should type `/pathly ...` or `/path ...` instead of `/help`, `/go`,
`/debug`, or `/explore` so Pathly does not collide with Codex built-in commands.
`/pathly` is canonical. `/path` is the short alias.
...
/path help                           -> Pathly help workflow
...
/path add password reset             -> director workflow
/pathly plan checkout-flow standard  -> plan workflow
/pathly verify-state checkout-flow   -> verify-state workflow
```
