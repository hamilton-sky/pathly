...
**Produces:** `plans/$FEATURE/` — 4 files in lite, all 8 plan files in standard/strict, pre-populated from the BMAD PRD
...
After writing files, verify the selected rigor's required files exist. For standard/strict, run:
```
...
```
BMAD import complete. Created plans/$FEATURE/.
